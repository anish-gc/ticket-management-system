...
class AccountSerializer(serializers.Serializer):
    username = serializers.CharField( error_messages={'required': 'Username cannot be blank.'})
    address = serializers.CharField(required=False)
    email = serializers.CharField(error_messages={'required': 'Email cannot be blank.'})
    phoneNumber = serializers.CharField(source="phone_number", error_messages={'required': 'Phone Number cannot be blank.'})
    ...
    def _check_unique_fields(self, data, exclude_id):
        ...
        if 'email' in data and Account.objects.filter(email=data["email"]).exclude(reference_id=exclude_id).exists():
