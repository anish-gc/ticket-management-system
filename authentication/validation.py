...
def confirm_login_details(user: Account) -> Dict[str, Any]:
    ...
    try:
        ...
        auth_data = {
            "accessToken": access_token,  # Keeping "token" field name for backward compatibility
            "refreshToken": refresh_token,
            "username": user.username,
            "designation": 'superadmin' if user.is_superuser else 'staff',
            "sessionTime": settings.SIMPLE_JWT.get("ACCESS_TOKEN_LIFETIME").total_seconds() / 60,
        }
        return auth_data
